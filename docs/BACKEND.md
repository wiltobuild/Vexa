...
Message triggers persist an outbox entry atomically with each mutation. A single advisory-locked publisher retries pending entries through an atomic Redis Lua script that assigns a sequence, appends to the replay stream and publishes. Gateway nodes independently subscribe to Redis. Retried batches may repeat events and clients reconcile them. No guaranteed exactly-once delivery or load/latency number is claimed.
