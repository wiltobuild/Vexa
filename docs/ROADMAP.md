...
| 1 Guilds & text | Guilds/channels, cursor-paginated REST chat, UI shell | CRUD/pagination coverage; browser creates guild and sends real message | Connected UI + REST implementation; live browser creates servers and sends messages against Postgres/gateway in CI. Broader CRUD/pagination coverage remains open. |
| 2 Gateway | Redis delivery, protocol, resume, typing, presence | Two-client real-time test; exact missed-event replay test | Delivery/resume integration test included; transactional message outbox implemented; presence fan-out open. |
| 3 Permissions & social | Roles/overrides, invites, friends/DMs, moderation, audit | Table-driven permission tests; denied-access test for every route/dispatch | Permission calculator, role CRUD/assignment (escalation-guarded), invite UI, moderation (kick/ban/timeout + audit log), and friends/1:1 DMs (request/accept/block, block closes an existing DM both ways) implemented and integration/e2e-tested; group DMs and the full denied-access matrix open. |
| 4 Rich messaging | Presigned uploads, thumbnails, safe Markdown, embeds/reactions | Upload round-trip; XSS and SSRF tests | Safe client Markdown + XSS browser check; shared demo/connected reaction picker with persistent counts and live delivery; connected pinned history with moderation checks and a concurrent-safe cap; local demo attachments; production upload pipeline open. |
...
| 6 Vexa Recall | Consented transcription and searchable history | Consent/permission/retention/worker tests; real audio-to-text demo | Personal Whisper notes implemented in demo (on-device) and connected mode (private history/retention); group-call consent and SFU worker gates remain open. |
